...
import { MockERC20, MockSwapRouter, PaymentGatewayV3 } from "../../typechain-types";
...
describe("PaymentGateway (TypeScript)", function () {
    ...
    beforeEach(async function () {
        ...
        mockRouter = await MockSwapRouterFactory.deploy(await idrx.getAddress());

        const PaymentGateway = await ethers.getContractFactory("PaymentGatewayV3");
    ...
    describe("Deployment", function () {
        it("Should set the correct owner, IDRX, and router addresses", async function () {
            ...
            expect(await gateway.swapRouter()).to.equal(await mockRouter.getAddress());
        });
